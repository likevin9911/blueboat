...
def neutral(m, secs=0.5):
    # ALWAYS flash forward before neutral (per your requirement)
    forward_flash(m)
...
def stop_all(m):
    print("==============================================================")
    print("[STOP] NEUTRAL + DISARM + CLEAR")
    print("==============================================================")
    try:
        neutral(m, 0.5)
        arm(m, False)
        # clear overrides a few times
        for _ in range(5):
            rc_override(m, None, None)
            time.sleep(0.05)
    except Exception:
        pass
